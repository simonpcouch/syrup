...
[![Codecov test coverage](https://codecov.io/gh/simonpcouch/syrup/branch/main/graph/badge.svg)](https://app.codecov.io/gh/simonpcouch/syrup?branch=main)
<!-- badges: end -->
...
We'll demo using the package with an (admittedly dependency-heavy) example of tuning a regularized linear model against resamples. First, loading needed packages:
...
Using future to define our parallelism strategy, we'll set `plan(multicore)`, indicating that we'd like to use forking with 5 workers. By default, future disables forking from RStudio; I know that, in the context of building this README, this usage of forking is safe, so I'll temporarily override that default with `parallelly.fork.enable`. 
...
The main function in the syrup package, `syrup()`, takes an expression as argument:
...
* Setting up another R process `sesh` that queries system information at a regular interval,
...
We can plot to get a better sense of how memory usage of these processes changes over time.
...
Note that memory is weird; in the above plot, the total memory allotted to the parent session and its five workers at each ID is not simply the sum of those `rss` values, as memory is shared among them.
